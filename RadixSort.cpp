...
void radixDesc(MultiKey<T>** mk, int n, int keylen, int low, int high){
    ...
    LinkQueue<MultiKey<T>*>** blanket = new LinkQueue<MultiKey<T>*>*[keylen];
    blanket[keylen-1] = new LinkQueue<MultiKey<T>*>[(high - low + 1)];
    for(int i=0;i<n;i++)
    {
        int keyVal = mk[i]->keys[keylen-1];
        blanket[keylen-1][keyVal - low].enQueue(mk[i]);
    }
    for(int k=keylen-2;k>=0;k--)
    {
        blanket[k] = new LinkQueue<MultiKey<T>*>[(high - low + 1)];
        for(int i=high;i>=low;i--)
        {
            while(!blanket[k+1][i - low].isEmpty())
            {
                MultiKey<T>* tmpMk = blanket[k+1][i - low].deQueue();
                int keyVal = tmpMk->keys[k];
                blanket[k][keyVal - low].enQueue(tmpMk);
            }
        }
    }
    int index = 0;
    ...
    {
        while(!blanket[0][j - low].isEmpty())
        {
            mk[index] = blanket[0][j - low].deQueue();
            index++;
        }
    }

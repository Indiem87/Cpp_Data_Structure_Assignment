...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void addEdge(int sIndex, int eIndex,int w = 1){
            if(adjMatrix[sIndex][eIndex]==0) numEdge++;
        ...
        LinkQueue<int>* findTopo(){ //directed graphs only
            ...
            if(numQ < numVer){
                ...
                LinkQueue<int>* p = new LinkQueue<int>;
                return p;
            }
            return q;
        }
        ...
        void PrizeSolver()
        {
            ...
            while(!q->isEmpty())
            {
                ...
                for(int i=0;i<numVer;i++)
                {
                    ...
                    {
                        int adjprize = 0;
                        adjprize = prize[i] + adjMatrix[i][current];
                        prize[current] = prize[current]>adjprize?prize[current]:adjprize;
                    }
                }
            }
            for(int i=0;i<numVer;i++)
            {
                cout<<left<<setw(10)<<i+1<<prize[i]<<endl;
            }
        }

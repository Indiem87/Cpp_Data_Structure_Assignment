...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        bool isBipartite(){ //ques 3
            ...
            for(int i=0;i<numVer;i++)
            {
                cout<<verList.getEleAtPos(i)<<": group "<<groups[i]<<endl;
            }
            return true;
        }
        ...
        bool loopTest(){
            ...
            else
            {
                ...
                while(!q.isEmpty()){
                    ...
                    popped++;
                    for(int i=0; i<numVer; i++){
                        if(!adjMatrix[tmp][i]) continue;
                        if(--indeg[i]==0) q.enQueue(i);
                    }
                }
                for(int i=0;i<numVer;i++) if(indeg[i]>0) return true;
                return false;
            }
        }
};

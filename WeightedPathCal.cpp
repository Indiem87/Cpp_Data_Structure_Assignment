...
class LinkStack{
    ...
    public:
        ...
        void push(T x){
            DNode<T>* tmp = new (std::nothrow) DNode<T>;
            if (!tmp) {
                throw std::bad_alloc();
            }
            tmp->data = x;
...
int weightedPathLength(BinTree<int>* bt){
    LinkQueue<BinTree<int> *> q;
    int layer = 0,w = 0,i = 0,num = 1,nextnum = 0;
    q.enQueue(bt);
    while(!q.isEmpty()){
        BinTree<int>* tmpN = q.deQueue();
        if(tmpN->isLeaf()) w += layer*tmpN->getData();
        i++;
        if(tmpN->getLChild()!=NULL){
        q.enQueue(tmpN->getLChild());
        nextnum++;
        }
        if(tmpN->getRChild()!=NULL){
        q.enQueue(tmpN->getRChild());
        nextnum++;
        }
        if(i == num)
        {

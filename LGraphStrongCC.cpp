...
class LGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    private:
        ...
        void DFShelper2(int st, bool* visited, LinkStack<int>& s)
        {
            visited[st] = true;
            graphNode<T>* tmpNode = verList.getEleAtPos(st);
            for(int i=0;i<tmpNode->outs->getLength();i++)
            {
                int target = verList.locateEle(tmpNode->outs->getEleAtPos(i));
                if(visited[target]) continue;
                DFShelper2(target,visited,s);
            }
            s.push(st);
        }

        void DFShelperGroup(int st, bool* visited, int* groupID, int currgroup){
            visited[st] = true;
            groupID[st] = currgroup;
            graphNode<T>* tmpNode = verList.getEleAtPos(st);
            for(int j=0; j<tmpNode->outs->getLength(); j++)
            {
                int target = verList.locateEle(tmpNode->outs->getEleAtPos(j));
                if(!visited[target])
                    DFShelperGroup(target, visited, groupID, currgroup);
            }
        }
    ...
    public:
        ...
        // question 1
        LGraph<T>* buildinverseGraph()
        {
            T* tmp = new T[numVer];
            for(int i=0;i<numVer;i++)
            {
                tmp[i] = verList.getEleAtPos(i)->data;
            }
            LGraph<T>* inverseG = new LGraph<T>(tmp,numVer,directed);
            for(int i=0;i<numVer;i++)
            {
                graphNode<T>* currNode = verList.getEleAtPos(i);
                for(int j=0;j<currNode->outs->getLength();j++)
                {
                    T targetData = currNode->outs->getEleAtPos(j)->data;
                    inverseG->addEdge(targetData,currNode->data);
                }
            }
            return inverseG;
        }
        ...
        void printStrongCC(){
            ...
            for(int i=0;i<numVer;i++)
            {
                if(!visited[i])
                {
                    DFShelper2(i,visited,NodeStack);
                }
            }
            LGraph<T>* inverseG = buildinverseGraph();
            for(int i=0;i<numVer;i++)
                visited[i] = false;
            int count = 0;
            int* groupID = new int[numVer];
            for(int i=0;i<numVer;i++)
                groupID[i] = -1;
            while(!NodeStack.isEmpty())
            {
                ...
                if(!visited[index])
                {
                    inverseG->DFShelperGroup(index,visited,groupID,count);
                    count++;
                }   
            }
            bool* groupPrint = new bool[count];
            ...
            {
                int tmp = groupID[i];
                if(tmp != -1 && !groupPrint[tmp])
                {
                    groupPrint[tmp] = true;
                    cout<<"Component "<<printed<<": ";
                    for(int j=0;j<numVer;j++)
                    {
                        if(groupID[j]==tmp)

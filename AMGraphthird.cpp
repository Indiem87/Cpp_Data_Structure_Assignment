...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        AMGraph<T>* subgraph(T* vList, int n){ //ques 1
            AMGraph<T>* tmp = new AMGraph<T>(vList,n,directed);
            for(int i=0;i<n;i++)
            {
                int sIndex = verList.locateEle(vList[i]);
                for(int j=0;j<n;j++)
                {
                    int eIndex = verList.locateEle(vList[j]);
                    if(adjMatrix[sIndex][eIndex])
                    {
                        tmp->addEdge(vList[i],vList[j]);
                    }
                }
            }
            return tmp;
        ...
        AMGraph<int>* getLineGraph(){
            int* newVerlist = new int[numEdge];
            int corrVer[numEdge][2];
            int count = 0;
            for(int i=0;i<numVer;i++)
            {
                for(int j=i+1;j<numVer;j++)
            ...
            AMGraph<int>* tmp = new AMGraph<int>(newVerlist,numEdge,false);
            for(int i=0;i<numEdge;i++)
            {
                for(int j=0;j<numEdge;j++)
                {
                    if(i==j) continue;
                    if(corrVer[i][1]==corrVer[j][0] || corrVer[i][0]==corrVer[j][1] || corrVer[i][0]==corrVer[j][0] || corrVer[i][1]==corrVer[j][1])
                    {
                        tmp->addEdge(i+1,j+1);
                    }
                }

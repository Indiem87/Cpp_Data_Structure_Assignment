...
class LTree{
    ...
    public:
        ...
        // problem 2
        void heightiter(int p,LTree* original)
        {
            p++;
            for(int i=0;i < children.getLength();i++)
            {
                children.getEleAtPos(i)->heightiter(p,original);
            }
            original->SeekHeight = original->SeekHeight>p?original->SeekHeight:p;            
        }
        int height(){
            SeekHeight = 0;
            heightiter(0,this);
        ...
        void degreeiter(LTree* original)
        {
            original->deg = original->deg>children.getLength()?original->deg:children.getLength();
            for(int i=0;i < children.getLength();i++)
            {
                children.getEleAtPos(i)->degreeiter(original);
            }
        }
        int degree(){
            deg = 0;
        ...
        void countNodeWithDegiter(int x,LTree* original){      
            if(children.getLength() == x)
            original->countx++;      
            for(int i=0;i < children.getLength();i++)
            {
                children.getEleAtPos(i)->countNodeWithDegiter(x,original);
            }
        }
        int countNodeWithDeg(int x){    
            countx = 0;        
            countNodeWithDegiter(x,this);
            return countx;
        }
        

        // problem 3
        void isDescendentOfiter(LTree* original)
        {
            for(int i=0;i < children.getLength();i++)
            {
                if(children.getEleAtPos(i) == original)
                {
                    original->Descendentjudge = true;
                    return;                
                }
                children.getEleAtPos(i)->isDescendentOfiter(original);   
            }                 
        }
        bool isDescendentOf(LTree<T>* lt){
            //test whether the current tree is a descendent of lt.
            Descendentjudge = false;
            lt->isDescendentOfiter(this);
            return Descendentjudge;
        }
        void isAncestorOfiter(LTree<T>* lt,LTree* original)
        {
            for(int i=0;i < children.getLength();i++)
            {
                if(children.getEleAtPos(i) == lt)
                {
                    original->Ancestorjudge = true;   
                    return;                 
                }
                children.getEleAtPos(i)->isAncestorOfiter(lt,original);
            }
        }
        bool isAncestorOf(LTree<T> * lt){
            //test whether current node is ancestor of lt.
            Ancestorjudge = false;
            isAncestorOfiter(lt, this);
...
LTree<int>* ques1(){
    ...
    // TODO
    LTree<int>* newTree00 = new LTree<int>(25);
    LTree<int>* newTree10 = new LTree<int>(15);
    LTree<int>* newTree11 = new LTree<int>(50);
    LTree<int>* newTree20 = new LTree<int>(10);
    LTree<int>* newTree21 = new LTree<int>(22);
    LTree<int>* newTree22 = new LTree<int>(70);
    LTree<int>* newTree30 = new LTree<int>(12);
    LTree<int>* newTree31 = new LTree<int>(66);
    LTree<int>* newTree32 = new LTree<int>(90);
    newTree00->addChild(newTree10);
    newTree00->addChild(newTree11);
    newTree10->addChild(newTree20);
    newTree10->addChild(newTree21);
    newTree20->addChild(newTree30);
    newTree11->addChild(newTree22);
    newTree22->addChild(newTree31);
    newTree22->addChild(newTree32);
    return newTree00;
}
/* Question 1 END */

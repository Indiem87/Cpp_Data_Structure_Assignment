...
/* Solution here */
int dpReturn(int i, int j, int** dp, int** cutpoint, int* sizes){
    if(j-i <= 1) return 0;
    if(dp[i][j]!=0) return dp[i][j];
    int minVal = -1;
    for(int k=i+1;k<j;k++)
    {
        int front = dpReturn(i,k,dp,cutpoint,sizes);
        int back = dpReturn(k,j,dp,cutpoint,sizes);
        int total = front + back + sizes[i]*sizes[k]*sizes[j];
...
void printParenthesis(int i, int j, int** cutpoint){
    if(j-i<=1)
    {
        cout << "(" << i << ")";
        return;
    ...
    cout << "(";
    printParenthesis(i, cutpoint[i][j], cutpoint);
    printParenthesis(cutpoint[i][j], j, cutpoint);
    cout << ")";
}
int minOpMatrixMult(int* sizes, int n){
    ...
    int result = dpReturn(0,n,dp,cutpoint,sizes);
    printParenthesis(0,n,cutpoint);
    cout << endl;
    for(int i=0;i<n+1;i++){
        delete[] dp[i];
        delete[] cutpoint[i];

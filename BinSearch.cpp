...
int missingAPterm(ArrayWrap<int> &aw){
    ...
    int start=0,end=length-1,dist;
    int ArrStart = aw.operator[](0),ArrEnd = aw.operator[](length-1);
    dist = (ArrEnd - ArrStart)/length;
    while(end-start > 1)
    {
        int middle = (end+start)/2;
        int ArrMid = aw.operator[](middle);
        int expected = ArrStart + (middle - start)*dist;
        if(expected == ArrMid)
        {
            start = middle;
            ArrStart = ArrMid;
...
int maxEleBitonic(ArrayWrap<int> &aw){
    ...
    // This will be observed via readCount.
    int length = aw.getLength();
    int ArrStart0 = aw.operator[](0),ArrEnd0 = aw.operator[](length-1);
    int ArrStart = aw.operator[](1),ArrEnd = aw.operator[](length-2);
    int start=1,end=length-2;
    if(ArrStart0 < ArrStart && ArrEnd0 > ArrEnd)
        return ArrEnd0;
    else if(ArrStart0 > ArrStart && ArrEnd0 < ArrEnd)
        return ArrStart0;
    else if(ArrStart0 > ArrStart && ArrEnd0 > ArrEnd)
        return ArrStart0 > ArrEnd0 ? ArrStart0 : ArrEnd0;
    while(end-start > 1)
    {
        int middle = (end+start)/2;
        int ArrMid = aw.operator[](middle);
        int ArrMidPlus = aw.operator[](middle+1);
        if(ArrMid < ArrMidPlus)
        {
            start = middle;
        }
        else
    ...
    return aw.operator[](start) > aw.operator[](end) ? aw.operator[](start) : aw.operator[](end);
